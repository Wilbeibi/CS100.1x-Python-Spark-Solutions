...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda log: getCountsAndAverages(log))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda log: ((log[0], log[1]), log[2]))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda log: ((log[0], log[1]), log[2]))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .mapValues(lambda ratings: (ratings[0] - ratings[1]) * (ratings[0] - ratings[1])))

    # Compute the total squared error - do not use collect()
    totalError = (squaredErrorsRDD.map(lambda log: log[1])
...
validationForPredictRDD = validationRDD.map(lambda log: (log[0], log[1]))
...
myModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations, 
                        lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda log: (log[0], log[1]))
...
trainingAvgRating = trainingRDD.map(lambda log: log[2]).reduce(lambda a, b: a + b) / float(trainingRDD.count())
                                
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda log: (log[0], log[1], trainingAvgRating))
...
print '(number of ratings, (movie name, number of reviews))'
...
# print movieIDsWithAvgRatingsRDD.filter(lambda log: log[1][0] > 1000).join(moviesRDD).mapValues(lambda log: log[1]).collect()
...
myRatedMoviesSet = set([log[1] for log in myRatedMovies])
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda log: log[0] not in myRatedMoviesSet)
                     .map(lambda log: (myUserID, log[0])))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda log: (log[0], log[1][0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda log: (log[1], log[2]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda log: (log[1][0][0], log[1][1], log[1][0][1]))
                       .filter(lambda log: log[2] > 75))
